<template lang="">
    <a-page-header
     ...
     title="Quản lý dữ liệu danh mục" @back="goBack">
     <template #extra>
...
<ModalCreate :parentId="$route.params.id"  @addSuccess="fetchData(pagination.current,pagination.pageSize,{parentId_Filter:$route.params.id})" ref="modalCreate" />
<ModalUpdate @updateSuccess="fetchData(pagination.current,pagination.pageSize,{parentId_Filter:$route.params.id})" ref="modalUpdate" />
</template>
<script>
  ...
  async mounted() {
     ...
     const selectedMenu = inject('selectedMenu');
      ...
      this.fetchData(this.pagination.current, this.pagination.pageSize,initParam);
    ...
    methods: {
      ...
      async deleteObj(id) {
        try {
          const response = await APIService.delete(`datacategory/soft-delete/${id}`);
          if (response.data.message == "Xóa thành công") {
            ...
            this.fetchData(this.pagination.current, this.pagination.pageSize,{parentId_Filter:this.$route.params.id}); // Tải lại dữ liệu
          } else {
      ...
      async fetchData(pageIndex, pageSize, params) {
        ...
        try {
          const response = await APIService.post('/datacategory/get-data-by-page', searchParam);
