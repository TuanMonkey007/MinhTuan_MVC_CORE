                ...
                <template #title>
                    <a-breadcrumb separator=">" style="margin-left:30px;">
                        ...
                        <a-breadcrumb-item class="active">
                            ...
                            <span style="color: #fff;">{{ this.categoryName }}</span>
                        </a-breadcrumb-item>
            ...
            <a-row justify="space-between" :gutter="[16,16]">
                    ...
                    <a-row justify="center">
                        ...
                        <a-col :span="24" style="display: flex; justify-content:start;">
                            ...
                            <div style="font-weight: 300;width: 100%; display: flex; justify-content:space-between">
                                <p style="font-weight: 300;">Tác giả: {{ this.article.author }}</p>
                                <div style="display: flex; align-items: center;">
                                    ...
                                    <font-awesome-icon icon="fa-regular fa-eye"
                                        style="color: #bccce6;margin-left: 10px;" />
                                    <span
                                        style="font-size: 12px; margin-left:5px;margin-right: 10px; align-items: center;">{{
                                            article.countView
                                        }}</span>
                                </div>
                ...
                <a-col :xs="24" :sm="24" :md="24" :lg="8" :xl="8">
                    <a-card style="background-color: #e6d8d8;">
                        <template #title>
                            <p style="font-weight: 500;font-size: 20px;">Danh mục: {{ this.categoryName }}</p>
                        </template>
                        <a-row v-for="blog in this.articleRelative" :key="blog.id">
                            <a-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
                                ...
                                max-height:100px;width: 160px">
                                                <div
                                                    style="width: 160; height: fit-content;overflow: hidden; max-height:100px">
                                            ...
                                            <a-col :xs="14" :sm="14" :md="16" :lg="16" :xl="16">
                                                <span style="font-size: 14px; align-items: center;font-weight: 600;">{{
                                                    blog.title }}</span>
                                                <p style="font-size: 14px; align-items: center;">{{
                                                    blog.summary.substring(0, 100) + (blog.summary.length > 100 ? '...'
                                                    :
                                                    '') }}</p>
                                                <font-awesome-icon icon="fa-regular fa-clock" style="color: #a3b0c7;" />
                                                <span
                                                    style="font-size: 12px; align-items: center;">{{ formatCreatedDate(blog.publishDate)
                                                    }}</span>
...
<script>
    ...
    export default {
        ...
        methods: {
            async fetchCategory() {
                try {
                   // const response = await APIService.get(`datacategory/get-data-by-code-and-parent-code/${this.$route.params.categoryCode}/BAI_VIET`);
                   // this.categoryName = response.data.data.name
                    const resCate  = await APIService.get(`datacategory/get-list-by-parent-code/BAI_VIET`)
                    this.categoryName = resCate.data.data.find(item => item.code == this.$route.params.categoryCode).name
                    this.categoryId = resCate.data.data.find(item => item.code == this.$route.params.categoryCode).id
                } catch (error) {
                    console.log(error)
                }
            },
            ...
            async fetchArticleRelative() {
                try {
                    ...
                    this.articleRelative = res.data.data.items;
